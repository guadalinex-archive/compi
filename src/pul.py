    ...
    def button_press(self, widget, event):
        ...
        if event.type == gtk.gdk.BUTTON_PRESS:
            if event.button==1:
                ...
                # copiar boton
                if widget.ismodificable and not widget.ispreview and not config.base.fullscreen:
                    global copy
                    if copy:
    ...
    def insertar(self):
        ...
        if self.config["enlace_a_mosaico"]:
            if os.path.exists(self.config["enlace_a_mosaico"]):
                ...
                config.base.abrir(None, self.config["enlace_a_mosaico"])
            elif os.path.exists(relative):
                print "Abriendo mosaico %s..." % os.path.split(relative)[1]
                config.base.abrir(None, relative)
            else:
